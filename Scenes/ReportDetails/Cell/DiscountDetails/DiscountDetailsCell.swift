...
class DiscountDetailsCell: UITableViewCell {
    ...
    func setupCell(item: ReportModels.DiscountDetailReportModel, index: Int, curr: HomeModels.Currencies, isBranch: Bool){
        ...
        self.branchLabel.text = isBranch ? item.indirimAdi : item.subeAdi
        self.cashierName.text = item.kasiyerAdi
        self.tableNo.text = "\(item.masaNo)"
        self.discountAmount.text = "\(item.indirimTutari.formatCurrency(currency: curr))"
